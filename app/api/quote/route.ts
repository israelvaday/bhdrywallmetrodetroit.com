...
export async function POST(req: Request) {
  ...
  const to = process.env.QUOTE_TO_EMAIL || "service@bhdrywallmetrodetroit.com";
  const from = process.env.QUOTE_FROM_EMAIL || "BH Drywall Metro Detroit <onboarding@resend.dev>";
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from,
        to: [to],
        subject: `Drywall quote — ${fields.service} — ${fields.name}${fields.urgency ? " (" + fields.urgency + ")" : ""}`,
        html,
        text,
...
function renderQuoteText(f: QuoteFields, count: number): string {
  return [
    "New quote request — BH Drywall Metro Detroit",
    "",
    `Name:     ${f.name}`,
    `Phone:    ${f.phone}`,
    `Email:    ${f.email || "—"}`,
        ...
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600" style="max-width:600px;width:100%;background:#0f172a;border:1px solid #1f2a44;border-radius:20px;overflow:hidden;box-shadow:0 25px 60px -20px rgba(0,0,0,0.6);">
          ...
          <tr>
            <td style="background:linear-gradient(135deg,#0b1220 0%,#1a2548 60%,#3a2a10 100%);padding:28px 28px 22px 28px;border-bottom:1px solid #b58a3a33;">
                ...
                <tr>
                  <td style="vertical-align:middle">
                    <div style="display:inline-block;background:#0b1220;border:1.5px solid #b58a3a;border-radius:14px;padding:8px 14px;">
                      <span style="font-family:Georgia,serif;font-size:15px;font-weight:bold;color:#e6c266;letter-spacing:0.5px">BH Drywall</span>
              ...
              <h1 style="margin:18px 0 4px 0;font-size:24px;line-height:1.25;color:#ffffff;font-weight:800;letter-spacing:-0.3px">New drywall quote request</h1>
              ...
              <table role="presentation" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  ...
                  ${
                    email
                      ? `<td style="padding:0 6px">
                          <a href="mailto:${email}?subject=Re:%20Your%20BH%20Drywall%20Metro%20Detroit%20quote" style="display:inline-block;background:transparent;color:#e6c266;text-decoration:none;font-weight:700;font-size:14px;padding:13px 20px;border-radius:12px;border:1.5px solid #e6c266">✉ Reply</a>
                        </td>`
                      : ""
                  }
                </tr>
              ...
              <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
                <tr>
                  <td style="font-size:11px;color:#6b7794;line-height:1.6">
                    <b style="color:#9aa6c1">BH Drywall Metro Detroit</b><br>
                    Licensed drywall contractor • Licensed & insured • Metro Detroit, MI<br>
                    <a href="https://bhdrywallmetrodetroit.com" style="color:#b58a3a;text-decoration:none">bhdrywallmetrodetroit.com</a>
                  </td>
                </tr>
              </table>
            </td>
